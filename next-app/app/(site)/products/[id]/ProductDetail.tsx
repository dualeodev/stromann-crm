            ...
            ? [
                {
                  ...
                  href: `/products?g=${primaryCategory.slug}`,
                },
              ]
            : []),
          { label: p.name },
      ...
      <section className='pd-hero'>
        ...
        <div className='pd-info'>
          <div className='pd-info__top'>
            {primaryCategory && (
              <Tag variant='brand'>{primaryCategory.name}</Tag>
            )}
          ...
          <div className='pd-info__ctas'>
            <Btn variant='primary' size='lg' href={`/quote?product=${encodeURIComponent(p.name)}`}>
              Yêu cầu báo giá
            </Btn>
            ...
            {p.documents.find((d) => d.kind === 'msds') && (
              <Btn
                variant='ghost'
                size='lg'
                href={p.documents.find((d) => d.kind === 'msds')!.url}
              >
                ↓ Tải MSDS
              </Btn>
            )}
          ...
          <div className='resource-grid'>
            {p.documents.length === 0 ? (
              <p className='text-n-600'>
                Chưa có tài liệu nào được đính kèm cho sản phẩm này.
              </p>
            ) : (
              p.documents.map((r) => (
