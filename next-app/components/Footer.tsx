...
import { listCategories, listProductIndustries } from '@/lib/catalog';

const linkCls =
  'text-n-400 text-[13px] transition-colors duration-150 hover:text-white';
...
function FooterCol({
  ...
  return (
    <div>
      <h5 className='text-white text-sm font-bold m-0 mb-4 tracking-[0.01em]'>
        {title}
      </h5>
...
export default async function Footer() {
  ...
  const productLinks: Array<[string, string]> = categories
    .slice(0, 6)
    .map((c) => [c.name, `/products?g=${c.slug}`]);

  const industryLinks: Array<[string, string]> = industries.map((i) => [
    i.name,
    `/products?i=${i.slug}`,
  ]);
  ...
  return (
    <footer className='bg-n-900 text-n-300'>
      <div className='grid grid-cols-[1.6fr_1fr_1fr_1fr_1fr] gap-12 px-20 py-16 max-lg:grid-cols-2 max-md:grid-cols-1'>
        <div>
          <Link href='/' className='flex items-center gap-2.5 mb-4'>
            ...
            <div>
              <div className='font-bold text-xl tracking-[-0.01em] text-white'>
                STROMANN
              </div>
          ...
          <p className='text-n-400 text-[13px] leading-[1.6] mt-4'>
            Công ty Cổ phần Stromann Việt Nam — phân phối phụ gia hiệu suất cao
            cho ngành sơn, mực in và nhựa. Đối tác chiến lược của MÜNZING (Đức).
          </p>
      ...
      <div className='border-t border-n-800 px-20 py-5 flex justify-between items-center text-xs text-n-500 max-md:flex-col max-md:gap-3'>
        <span>
          © {new Date().getFullYear()} Stromann Việt Nam. All rights reserved.
        </span>
